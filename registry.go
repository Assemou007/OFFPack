...
func fetchPackageMeta(name string) (*PackageMeta, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("package %s introuvable (HTTP %d): %s", name, resp.StatusCode, body)
	}
...
func downloadTarball(url, integrity string) ([]byte, error) {
	resp, err := http.Get(url)
	if err != nil {
...
func verifyIntegrity(data []byte, integrity string) error {
	...
	if len(parts) != 2 {
		return fmt.Errorf("intégrité npm invalide: %s", integrity)
	}
	expected, err := base64.StdEncoding.DecodeString(parts[1])
	if err != nil {
		return fmt.Errorf("empreinte npm invalide: %w", err)
	}
	var actual []byte
	switch parts[0] {
	case "sha512":
	...
	case "sha1":
		...
		actual = sum[:]
	default:
		return fmt.Errorf("algorithme d'intégrité npm non supporté: %s", parts[0])
	}
	if len(actual) != len(expected) || subtle.ConstantTimeCompare(actual, expected) != 1 {
		return fmt.Errorf("échec de vérification d'intégrité du tarball")
	}
	return nil
}
