...
func wildcardConstraint(raw string) (semver, int, bool) {
	...
	if wild < 0 && len(parts) < 3 {
		wild = len(parts)
	}
	if wild < 0 {
		return semver{values[0], values[1], values[2]}, 3, true
	}
...
func satisfiesComparator(v semver, token string) bool {
	...
	if token == "" || token == "*" || strings.EqualFold(token, "latest") {
		return true
	}
	if strings.HasPrefix(token, "^") {
		base, ok := parseSemver(strings.TrimSpace(token[1:]))
		if !ok {
			return false
		}
	...
	if strings.HasPrefix(token, "~") {
		base, ok := parseSemver(strings.TrimSpace(strings.TrimLeft(token[1:], " ")))
		if !ok {
			return false
		}
	...
	for _, op := range []string{">=", "<=", ">", "<", "="} {
		if strings.HasPrefix(token, op) {
			base, ok := parseSemver(strings.TrimSpace(token[len(op):]))
			if !ok {
				return false
			}
			c := cmpSemver(v, base)
	...
	base, precision, ok := wildcardConstraint(token)
	if !ok {
		return false
	}
	if precision == 3 {
...
func satisfiesRange(v semver, constraint string) bool {
	for _, alternative := range strings.Split(constraint, "||") {
		...
		if strings.Contains(alternative, " - ") {
			p := strings.SplitN(alternative, " - ", 2)
			a, aok := parseSemver(strings.TrimSpace(p[0]))
			b, bok := parseSemver(strings.TrimSpace(p[1]))
			if aok && bok && cmpSemver(v, a) >= 0 && cmpSemver(v, b) <= 0 {
				return true
			}
			continue
		}
...
func resolveBestVersion(versions []string, constraint string) string {
	...
	for _, raw := range versions {
		if v, ok := parseSemver(raw); ok && !strings.Contains(raw, "-") {
	...
	for i := len(candidates) - 1; i >= 0; i-- {
		if satisfiesRange(candidates[i].parsed, strings.TrimSpace(constraint)) {
			return candidates[i].raw
		}
	}
	return ""
}
